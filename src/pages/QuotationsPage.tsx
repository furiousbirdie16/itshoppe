...
import { Plus, Trash2, ArrowUp, ArrowDown, Eye, ArrowRight, FileText, FileDown, Pencil, Filter, Undo2, Lock } from "lucide-react";
                    ...
                    <div key={idx} className="border rounded-md p-2 sm:border-0 sm:p-0 sm:py-1">
                      ...
                      {lineTotal(line) > 0 && (
                        <p className="mt-1 text-[11px] text-muted-foreground text-right tabular-nums">
                          {parseQty(line.quantity)} × {peso(parsePrice(line.unit_price))} = <span className="font-medium text-foreground">{peso(lineTotal(line))}</span>
                        </p>
                      )}
